...
def is_end_business_month(date):
    #return (date.month+1)==(date + timedelta(days=1)).month
...
class TacticalAllocation:
    ...
    def ivy(df):
        """
            20% VTI,20% VEU,20% VNQ,20% AGG and 20% DBC
        """
        risky_assets_ls = ['VTI','VEU','VNQ','AGG','DBC']
        universe_df = df.filter(regex=r'(VTI|VEU|VNQ|AGG|DBC)')
        returns_df = returns(universe_df)
        #returns_df['Cash'] = returns_df.shape[1]/returns_df.isnull().sum(axis=1)
        #returns_df['Cash'] = np.where(returns_df['Cash']<=1,returns_df['Cash'],0)
        #pdb.set_trace()
        ...
        try:
            ...
            if abs(1 - sum(weights_ls)) < 1e-8:
                pass
        universe_monthly_df = universe_df.resample('BM').last()
        universe_10M_rolling_mean_df = (universe_monthly_df > universe_monthly_df.rolling(window=10).mean()).astype(int)
        ...
        # price = universe_df.values
        quantity = pd.DataFrame(index=returns_df.index,columns=returns_df.columns)
        initial_investment = 1
        idx = 0
        for date, series_s in universe_df.iterrows():
            if is_end_business_month(date):
                target_weight = pd.Series(np.where(series_s.isnull(), 0, (returns_df.shape[1]) ** (-1)),\
                                          index=series_s.index)
                target_weight = target_weight * universe_10M_rolling_mean_df.loc[date, :]
                if target_weight.sum()>0:
                    pdb.set_trace()
                if date == universe_10M_rolling_mean_df.index[0]:
                    quantity.loc[date, :] = (initial_investment * target_weight / series_s).fillna(0)
                else:
                    portfolio_value = (quantity.loc[returns_df.index[idx - 1], :] * series_s).sum()
                    print(portfolio_value)
                    quantity.loc[date, :] = (portfolio_value * target_weight / series_s).fillna(0)
                # if portfolio_value <= 1e-4:
                #     portfolio_value = 1
                # if date > datetime(2021,1,1):
                #     pdb.set_trace()
                # else:
                #     target_weight = pd.Series(np.where(series_s.isnull(),0,(returns_df.shape[1])**(-1)),index=series_s.index)
                #     target_weight = target_weight*universe_10M_rolling_mean_df.loc[date,:]
                #     portfolio_value = (quantity.loc[returns_df.index[idx-1],:] * series_s).sum()
                #     quantity.loc[date,:] = (portfolio_value*target_weight/series_s).fillna(0)
            else:
                if idx > 0:
                    quantity.loc[date,:] = quantity.loc[returns_df.index[idx-1],:]
            idx +=1
        pdb.set_trace()
        return (quantity.mul(universe_df)).sum(axis=1)+1
    ...
    def robust_asset_allocation_balanced(df):
        ...
        universe_df = df.filter(regex=r'(VNQ|^IEFA$|MTUM|IWB|^EFA$|EFV|^IEF$)')
        risk_free_asset_s = universe_df['BIL']
        returns_df = returns(universe_df)
        universe_df = universe_df[risky_assets_ls]
