...
from src.tradingDashboard import Data
...
class TestQuery(TestCase):
    ...
    def test_write_query_equity_curves_buy_and_hold(self,allocation='buy_and_hold'):
        ...
        self.assertEqual(TestQuery.data_obj.write_query_equity_curves(allocation=allocation),query_target)
    ...
    def test_write_query_returns(self,allocation='buy_and_hold'):
        ...
        self.assertEqual(TestQuery.data_obj.write_query_returns(allocation=allocation), query_target)

    def test_write_query_performance(self,allocation='buy_and_hold'):
        query_target = f'SELECT * FROM \"{allocation}_performance\";'
        self.assertEqual(TestQuery.data_obj.write_query_performance(allocation=allocation), query_target)

    def test_write_query_symbol_single(self,symbol='BIL'):
        query_target = f'SELECT \"{symbol}\",\"index\" FROM price;'
        self.assertEqual(TestQuery.data_obj.write_query_symbol(symbol=symbol),query_target)
    ...
    def test_write_query_strategies(self,allocation='buy_and_hold'):
        ...
        self.assertEqual(TestQuery.data_obj.write_query_strategies(),query_target)


if __name__ == '__main__':
    test_data_obj = TestQuery()
